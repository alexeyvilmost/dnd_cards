...
- Бэкенд: `https://bagofholding.ru` — задеплоен, включает
  A/B (миграции 040/041, роуты v3, feat related_*).
- Фронтенд: `https://bagofholding.ru`.
...
1. Историческая Фаза C не была выложена старым auto-deploy. Текущий Timecloud
   процесс выкладывает backend и frontend вместе через один exact-SHA runner.
