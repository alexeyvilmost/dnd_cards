# Исторический план: конструктор персонажа + система хранения
...
> **Статус терминологии:** документ описывает исторический двухклассовый
> предшественник micro-MVP. Каноническая текущая граница micro-MVP — семь
> классов и полностью исполняемая механика первого уровня — определена в
> [актуальном roadmap](./micro-mvp-roadmap.md). Этот план не является
> самостоятельным критерием приёмки.

---

## 1. Цель и рамки исходного двухклассового среза
...
### Содержимое исходного среза (обязательный набор данных)
...
4. **Лист персонажа** — новый минимальный лист для исходного среза (не переиспользуем
   4000-строчный `CharacterDetailV3`).

### Явно вне рамок исходного среза
...
proficiencyBonus(lvl) = 2 (исходный срез, lvl 1)
...
8. **Заклинания*** (добавить к макету) — выбор заговоров и заклинаний из `spells`
   ...
   (или подсвечивать для кастеров — Волшебник); в исходном срезе доступно всем.
...
- Загрузка сущностей: `classesApi/racesApi/backgroundsApi/featsApi/spellsApi`,
  фильтрация к набору исходного среза (по `card_number`/названию — см. §8).
...
## 7. Лист персонажа (минимальный, исходный срез)
...
## 8. Контент исходного среза (данные в БД)
...
- **Классы:** Воин, Волшебник. Нужны:
  ...
  - `level_progression["1"].effects/actions`: Воин — «Второе дыхание» (action),
    ...
    «Магическое восстановление» (по мере готовности; для исходного среза достаточно
...
14. Сидер контента исходного среза (классы с умениями, предыстории, черты) + проверка эльфа/
    подвидов.
...
после Фазы E (полный сквозной сценарий в браузере на Timecloud).
...
## 11. Дальнейшее расширение (после исходного среза)
