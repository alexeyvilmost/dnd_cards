...
Состояние поставки на 2026-08-03:

- CI на каждый push/PR обязательно выполняет production build, lint, 841+
  frontend unit-тест, offline MVP и backend-тесты; живой production-контент
  проверяется отдельным ночным/ручным gate;
- эталонный manifest, exhaustive-матрица и сертификация контента реализованы;
...
6. Нет автоматической сертификации любой допустимой комбинации эталонного
   каталога.
7. ~~Короткий отдых автоматически лечит половину максимальных HP.~~ Закрыто
   2026-08-03: выбор и расход костей хитов реализованы.
...
Статус на 2026-08-03: основной сквозной срез MM4 реализован.
...
До полного закрытия MM4 остаются: раскладывать само значение характеристики по
конкретным постоянным источникам (распределение, предыстория, предмет,
пользовательский метод) и автоматизировать desktop UI/E2E-приёмку popover.
...
положительного/отрицательного/граничного покрытия за пределами эталонного среза.
