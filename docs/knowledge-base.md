...
| KB-004 | **Кириллическая формула `'12 + ЛВК'` роняет весь лист в белый экран.** Токенизатор ASCII-only (`formula.ts:159`) → `FormulaError` не пойман ни `computeAC`, ни `breakdownAC`; **ErrorBoundary в приложении нет вовсе** → снять предмет через UI невозможно. Карта `3534fc69`. Корень: `ABILITY_LABEL_RU` печатает «ЛВК» и провоцирует ввод её же. Тем же ломаются 7 карт с костью «1к6» в `evaluate()` | critical | S | code+data | ЗАКРЫТО 0.1→next | AC-4 |
