...
| KB-027 | **«Помощь» `2863e54d` существует с рабочей механикой, но `type=null`** → фильтр листа её отбрасывает; grant_action-моста нет. Одно поле = действие на листе | high | S | data | ЗАКРЫТО 35382ab | ACT-4/F2 |
...
| KB-033 | **Ни один класс не выдаёт Боевой стиль**: строки «Боевой стиль» нет ни в одном классе, `type='Боевой стиль'` нет ни у одного эффекта, 5 стилей — сироты → Воин/Следопыт/Паладин не получают стиль совсем | high | S | data | ОПРОВЕРГНУТО 35382ab (§8.5) | I2 |
...
| KB-052 | **Ярость: сопротивление дроб./кол./руб. и преимущество на СИЛ — только текст.** Исполним лишь бонус урона. `kind:'resistance'` — HANDLED, используется 26 эффектами | high | S | data | ЗАКРЫТО f95c83e | CLS-4 |
| KB-053 | **Заряды Ярости зафиксированы на 2 для всех 20 уровней** (RAW 2/3/4/5/6). Соседние ресурсы того же файла уже используют `by_level`. **Второй хардкод той же двойки — `mechanics.uses` действия `ACT-rage`**: правка только `classes.json` молча не сработает. Реестр канона уже знает: `officials/canon/phb2024/class-barbarian.json:35` печатает «БАГ» | high | S | data | ЗАКРЫТО 5d247eb | CLS-5/B6 |
| KB-054 | **Вдохновение барда считается от `prof_bonus` вместо мод. Харизмы.** Знак ошибки переворачивается: L1 ХАР+3 → 2 вместо 3; L17-20 завышение. Правка `count:'max(cha,1)'` **синхронно в 2 местах** (`classes.json` + `mechanics.uses` действия `507a13bf`) | high | S | data | ЗАКРЫТО 5d247eb | CLS-6/B5 |
| KB-055 | **Четыре фичи стоят не на своём уровне**: Первобытные знания L1→L3, Фокус монаха L1→L2, Ловкий исследователь L1→L2, Мистические воззвания L2→L1 | high | S | data | ЗАКРЫТО 28a2021 | CLS-7 |
...
| KB-077 | **«Ускорение» `1661dfe1` даёт преимущество на ВСЕ спасброски** вместо только Ловкости; соседний narrative того же заклинания это признаёт. **Самоусиливающийся** — даёт преимущество и на ТЕЛ-спас концентрации, удерживающий само Ускорение. Фикс: `+ filter:{ability:'dex'}` | high | S | data | ЗАКРЫТО e012195 | SPL-6 |
| KB-078 | **«Паутина» `0446564e` наносит 2к4 огня каждому провалившему и масштабирует апкастом** — по RAW урона нет вовсе и апкаста у Web нет. Плюс `restrained` без `duration` → вечное состояние. Собственное описание карточки описывает правило **верно** | high | S | data | ЗАКРЫТО e012195 | SPL-7 |
...
| KB-082 | **«Огненный снаряд» `50626b5a` растёт костью 1d6 вместо RAW 1d10** — единственное расхождение среди 16 масштабируемых заговоров. На L17: 16.0 против 22.0 (−27%) на самом частом заговоре игры. Битые оба поля | medium | S | data | ЗАКРЫТО e012195 | SPL-11 |
...
| KB-112 | **Бонусы характеристик предыстории (+2/+1) можно молча пропустить**: `bonusIssues` на пустом распределении возвращает `[]`. Один клик по «+1/+1/+1» или по чипам обнуляет `assignments` → «Создать» активна, персонаж на −3 | medium | S | code | ЗАКРЫТО c08293b | CRT-4 |
| KB-113 | **Смена предыстории не переприменяет бонусы**: Солдат (str+2/dex+1) → Мудрец (con/int/wis) оставляет str/dex, обе вне списка Мудреца. Достигается обычной сменой предыстории. **Чинить одной точкой с KB-112** | medium | S | code | ЗАКРЫТО c08293b | CRT-5 |
| KB-114 | **Максимум хитов может стать отрицательным** — нет RAW-клампа «минимум 1 за уровень». d6/ТЕЛ 1/L5 → **−3**, персонаж создаётся мёртвым, бэкенд не валидирует. Фикс: `l1 + (lvl-1)*Math.max(1, perLevelAvg + conMod)` | low | S | code | ЗАКРЫТО c08293b | CRT-6 |
...
| KB-130 | **Линтера текстовых ссылок не существует**; 190 ссылок → 25 целей, 176 на 13 UUID состояний. `scripts/audit-refs.mjs` существует, но `collectRefs` парсит только `grant_spell`/`type:'feat'`. **Нет гейта на удалении**: `DeleteEffect` — голый `db.Delete` без проверки входящих; обратного индекса нет. **СЦЕНАРИЙ УЖЕ РЕАЛИЗОВАЛСЯ**: `9078764` — чистка приняла эффекты слотов черт за дубликаты. Фикс: расширить `collectRefs` регуляркой. Инфраструктурный блокер KB-131 закрыт | medium | S | infra | CONFIRMED | H10 |
| KB-131 | **Экспорт каталога закрыт fail-closed.** `export-prod.mjs` теперь выгружает `/api/concepts`; `resources`/`variables`/`concepts` поддерживают стабильную server-side пагинацию, а клиент ограничивает `maxPages`/`maxItems`, требует согласованный `total`, непустой строковый `id` и запрещает дубли между страницами. Отдельного `/api/conditions` намеренно нет: состояние — единственный авторитетный Effect с `effect_type='condition'`. Перед заменой снапшота exporter требует ровно одну Effect-строку для каждого `card_number` из фиксированного required-set 15 состояний; дополнительные будущие состояния допустимы. Concept-ссылки по-прежнему резолвятся по slug `concept_id`, не по row `id` | medium | S | infra | ЗАКРЫТО | H12 |
...
| id | Находка | sev | effort | kind | verdict | трек |
|---|---|---|---|---|---|---|
| KB-192 | **Закрыто 2026-08-03:** удалены доказанно мёртвые `hooks/useCardTilt.ts`, `utils/characterCalculationsV3.ts`, `utils/dndSuBestiary.ts`, его тест и две фикстуры. Живой `ttgClubBestiary` и `ttg-skeleton.html` сохранены. | low | S | code | CLOSED | 5.1 |
...
**Волна 1b — данные + мелкий код:** KB-001+KB-003 (**одной правкой**, иначе перебор) · KB-006 · KB-026 · KB-027 · KB-051 · KB-079 · KB-080 · KB-021 (**выписать модификаторы**) · KB-037/038/039/042 · KB-112/113 · KB-114 · KB-097 · KB-070 · KB-137 (**сначала enum**) · KB-092 · KB-130 (его инфраструктурный блокер KB-131 закрыт).
...
1. **Решение о плоских +50% HP отменено владельцем 2026-08-03 в пользу официальных правил 2024.** Короткий отдых теперь предлагает добровольно тратить кости хитов по одной, добавляет ТЕЛ (минимум 1 HP), сохраняет решение атомарно; длинный отдых восстанавливает пул. Абстрактная стоимость `hit_die` резолвится в фактический пул `hit_dice_dN`, поэтому KB-047 закрыт.
...
16. **«Для состояний нужен отдельный `/api/conditions`»** — состояния уже являются Effect и выгружаются в `effects.json`; второй endpoint создал бы конкурирующий источник истины. `/api/concepts` теперь входит в снапшот, а фиксированный required-set из 15 condition `card_number` проверяется до его атомарной записи (KB-131).
...
20. **KB-033 «Боевой стиль ни к одному классу не подключён; стили — untagged-эффекты»** — ОПРОВЕРГНУТО при исполнении (`35382ab`). Стиль полностью подключён: 10 черт `category='fighting_style'` (FEAT-0054..0063) + контейнеры `choice{source:'feat', filter:'fighting_style'}` в LP Воин L1 / Паладин L2 / Следопыт L2; резолвер фильтрует по category (`character/components.tsx:67-97`), есть тест `fightingstyle.forge.mvp.test.ts`. Правка плана (тег эффектов `type='Боевой стиль'` + choice `source:'effect_type'`) создала бы **ПАРАЛЛЕЛЬНЫЙ дублирующий пикер** — регресс. Аудитор принял 10 orphan-эффектов `fs_*` (дубли черт) за источник стилей. Реальный остаток здесь — только чистка orphan `fs_*` (dead-код, отдельно; грабли дедупа).
