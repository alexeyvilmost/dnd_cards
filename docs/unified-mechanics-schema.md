...
// Дать доступ к библиотечным действиям (Хитрое действие). Экономика (bonus_action) и поведение —
// на КАРТЕ каждого действия; grant_action несёт только ссылку value | values (slug'и).
{ "kind": "grant_action", "values": ["dash","disengage","hide"] }
...
| Плут: Хитрое действие | passive | grant_action(values: dash/disengage/hide) |
