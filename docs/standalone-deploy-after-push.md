...
на Timecloud Linux-сервер. Она рассчитана на уже подготовленный сервер и
повторяемый выпуск новой версии из `main`.
...
9. На сервере остаются только пять последних runnable-релизов. Активный релиз
   всегда входит в retention-набор; соответствующие архивы, распакованные build
   trees и application image tags удаляются вместе с более старыми релизами.
...
2. сокращает предыдущую историю до четырёх runnable-релизов, не трогая активный
   rollback и уже загруженный архив нового SHA;
3. создаёт custom-format дамп PostgreSQL;
4. собирает два Docker image с тегом Git SHA;
5. создаёт новый immutable release directory;
6. меняет `SOURCE_COMMIT` и symlink `current`;
7. ждёт health-check всех контейнеров;
8. проверяет SHA через публичный HTTPS;
9. удаляет build-артефакты и application image tags за пределами последних пяти
   релизов;
10. автоматически возвращает старый релиз при ошибке.
