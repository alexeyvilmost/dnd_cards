...
## 13:35 integration addendum

This addendum supersedes the earlier unsupported-row counts where the later work is more specific.

- Mounted Combatant and Polearm Master now have bounded executable primary paths. Polearm Master owns a provenance-preserving post-Attack Bonus Action and an enter-reach reaction; Mounted Combatant owns an explicit rider/mount relation and the larger-mount melee Advantage branch. Both remain `untested`: Mounted Combatant attack redirection, mount Evasion and controlled-mount economy are still absent, and Polearm Master's sheet-only post-Attack workflow is not yet proven.
- Wizard Memorize Spell is now a data-owned, optional short-rest preparation decision instead of a dead granted action. Browser QA on retained Wizard `ded98719-b42d-489f-9917-7516c1c425df` replaced Magic Missile with Grease, showed a clear journal entry, persisted the choice after reload, and projected Grease enabled/Magic Missile disabled in a newly constructed combat.
- A retained production Sorcerer `5097609d-27ca-4ca6-8acb-cf094d54be47` reached level 5 with Draconic Sorcery. The sheet showed 5 Sorcery Points, 4/3/2 slots, DC 14 and +6 spell attack. This exposed two strict catalog defects: Fly and Slow lacked stable `spell_class_list_ids`, causing the entire spell action projection to fail. Migration 190 repairs both. The first production retest then exposed a second, independent fail-closed invariant: subclass-emitted Fly/Fear grants lacked an explicit casting ability. Migration 193 stamps Charisma on all six Draconic Sorcery grants. Sorcerous Restoration is also moved from an unrestricted visible action to a short-rest trigger that restores floor(Sorcerer level / 2) once per Long Rest; focused regression proves the cap and repeat gate. Final production browser evidence is recorded after the follow-up release.
- The exact post-migration spell audit now compiles 115/115 level-2/3 PHB rows and 115/115 strict targeting declarations. All referenced effects/conditions resolve to library entities and no generic inline condition remains. This proves structural executability, not full spell-rule certification.
...
## 14:22 closure addendum

- Retained local Aasimar Warlock `4a12fae4-9da6-40df-b624-f8dd0f4fc286` reached level 3 through Forge and selected the Fiend patron plus Cloud of Daggers. Celestial Revelation now accepts the exact `emanation` geometry in the strict schema and preflight executor.
- Sheet execution passed: the explicit Inner Radiance choice spent one Bonus Action and the exact ability charge atomically, then displayed real library-backed `Внутренний свет` and `Небесное откровение` effect cards with source information.
- A fresh two-Goblin combat passed: the action required no false map destination, drew a 10-foot area centered on the Aasimar, labeled it as source-following and as affecting everyone inside at the source's turn end, and decremented from 10 to 9 rounds. Both Goblins took 2 radiant damage in separate canonical log entries; the Aasimar's own 2 damage was visibly reduced to 1 by Celestial Resistance. Moving the source re-centers the footprint and recalculates entry/exit events.
- Migration 194 adds strict executable primary paths for Berserker Frenzy and Tides of Chaos, plus the missing atomic result-level resource-spend vocabulary and attack-query facts. Migration 195 adds a deliberately bounded owned-summon lifecycle for Find Steed and Summon Beast/Fey/Undead. These entities remain `untested` pending their own complete sheet/combat/clarity browser scenarios.
- The combined area/resource/subclass/summon focused gate passed 53/53 tests. Full release gates and Timecloud publication are recorded below when complete.
- Independent cross-feature audit found and closed four additional release-significant paths: Radiance now snapshots the source proficiency bonus instead of evaluating the target's; teleport, push and pull re-anchor source-following areas; summon removal clears grapples, source-owned areas and queued triggers; persisted summon ownership and source-area footprints are normalized fail-closed before use.
- Final settled checks passed: 139 world-migration/solo-combat tests with one intentional skip, production TypeScript/build, strict lint, full backend tests/vet, and two-pass PostgreSQL replay of migrations 194–196. The final Timecloud SHA and five-release retention result are appended after publication.
- Remaining honest gaps are unchanged in certification terms: summon form-specific attacks/riders and command rules, Wings aerial traversal, Shroud's initial fear burst, and strict schema validation for every optional `world_zone.tactical` field. These rows remain `untested`; the new primary runtime paths are not represented as full entity certification.
- The current sheet/combat certificate drift check passes. The older `test:micro:manifest` bundle is independently red because committed `micro-mvp-l1-content-patch.v1.json` hashes to `sha256:633fafd2…` while its reviewed preimage remains pinned to `sha256:31148b36…`; 42 tests cascade from that single preflight mismatch. This batch did not change either legacy file, and the reviewed preimage was not silently regenerated under deadline.
- Production release `6612461bc6c32307f4a769e7edbb1ae38f32b868` is live. Public backend health, frontend build info, Timecloud `current`, and backend/frontend image tags all report that exact SHA; retention is exactly five releases. The public action catalog postimage contains `anchor=source` and `trigger_scope=source_turn_all_inside` for `ACT-aasimar-revelation`.
- The authenticated production sheet loaded successfully after cutover. The retained Aasimar is level 2 in production (its level-3 Forge state exists only in the isolated local clone), so the exact Radiance browser execution evidence remains the local production-shaped scenario recorded above rather than a fabricated production pass.
