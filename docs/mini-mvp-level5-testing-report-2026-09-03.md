...
- A retained production Sorcerer `5097609d-27ca-4ca6-8acb-cf094d54be47` reached level 5 with Draconic Sorcery. The sheet showed 5 Sorcery Points, 4/3/2 slots, DC 14 and +6 spell attack. This exposed two strict catalog defects: Fly and Slow lacked stable `spell_class_list_ids`, causing the entire spell action projection to fail. Migration 190 repairs both. Sorcerous Restoration is also moved from an unrestricted visible action to a short-rest trigger that restores floor(Sorcerer level / 2) once per Long Rest; focused regression proves the cap and repeat gate. Production retest remains pending until the final release is deployed.
