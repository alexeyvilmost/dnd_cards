...
Умножение с костью **асимметрично**:
- `N * d4` / `prof_bonus * martial_arts_die` — бросить кость **N раз** (увеличить count);
- `d4 * N` / `martial_arts_die * prof_bonus` — бросить кость один раз, **умножить сумму** на N.

Также: `prof_bonus d4` (без `*`) — то же, что N раз d4 (синтаксис scaling).
