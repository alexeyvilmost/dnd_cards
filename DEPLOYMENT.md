...
Runner автоматически сохраняет только пять последних runnable-релизов вместе
с соответствующими build-артефактами и Docker image tags. Перед созданием нового
релиза история сокращается до четырёх (активный релиз всегда защищён), а после
успешной публичной проверки фиксируется ровно пять доступных версий.
