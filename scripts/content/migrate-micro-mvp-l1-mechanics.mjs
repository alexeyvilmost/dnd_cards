...
const REVIEWED_SUPERSEDING_POSTIMAGES_PATH = join(
  REPO_ROOT,
  'frontend/src/canon/data/micro-mvp-reviewed-superseding-postimages.v1.json',
);
...
// must not be downgraded to the older postimage. The compiler and production
// migrator read this same registry; any unlisted hash fails closed.
const reviewedSupersedingPostimages = JSON.parse(
  readFileSync(REVIEWED_SUPERSEDING_POSTIMAGES_PATH, 'utf8'),
);
const reviewedHashes = [
  ...Object.values(reviewedSupersedingPostimages.mechanicsHashes ?? {}),
  ...Object.values(reviewedSupersedingPostimages.fieldHashes ?? {}),
];
if (reviewedSupersedingPostimages.schemaVersion !== 1
    || reviewedHashes.some((hash) => !/^sha256:[0-9a-f]{64}$/.test(hash))) {
...
export const REVIEWED_SUPERSEDING_MECHANICS_HASHES = Object.freeze({
  ...reviewedSupersedingPostimages.mechanicsHashes,
});
export const REVIEWED_SUPERSEDING_FIELD_HASHES = Object.freeze({
  ...reviewedSupersedingPostimages.fieldHashes,
});
