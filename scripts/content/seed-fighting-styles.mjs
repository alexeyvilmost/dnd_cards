 ...
 *     card_number fs_*): имя = имя черты («Оборона»), Оборона — исполнимый
 *     modifier +1 к КЗ, остальные — narrative с точным текстом правила (движок не
 *     умеет фильтры по типу оружия/перебросы, поэтому не выдумываем исполнимые
 *     модификаторы). Префикс «Боевой стиль:» в имени НЕ ставим — иначе на листе
 *     выглядит как способность класса, а в кузне пикер показывает просто «Оборона».
...
const STYLES = [
  ...
  },
  {
    ...
    // Единственный стиль с исполнимой механикой: +1 КЗ, только пока надет доспех.
    result: [
      {
        kind: 'modifier',
        applies_to: { roll: 'ac' },
...
async function ensureStyleEffect(token, s, effectsBySlug) {
  ...
  // Имя эффекта = имя черты («Оборона»), без префикса «Боевой стиль:» —
  // иначе на листе выглядит как отдельная способность класса, а в кузне
  // пикер показывает просто «Оборона» (категория fighting_style).
  const name = s.feat;
