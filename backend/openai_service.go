...
func GenerateImagePrompt(cardName, description, rarity string) string {
	...
	// Добавляем стиль
	prompt += "\nБез дополнительных элементов, рамок и прочего. Самое главное - предмет. Фон — прозрачный, без лишних деталей. Стиль — полуреалистичная отрисовка, мягкое освещение, лёгкое свечение по контуру, яркие акценты на металле, дереве или камне. Качество — как иконка предмета из RPG, детализированное, но с упрощённым прозрачным фоном, чтобы предмет выделялся.\nОбязательно на прозрачном однотонном фоне"
