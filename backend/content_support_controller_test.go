...
func TestValidateContentSupportRequest(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "valid mini MVP evidence and lock",
			request: ContentSupportRequest{
				Status:               "verified_mechanical",
				CertificationVersion: contentSupportString(miniMVPEvidenceCertificationVersion),
				ContentHash:          &validHash,
				DependencyHash:       &validDependencyHash,
				CertifiedAt:          contentSupportString("2026-08-20T07:00:00Z"),
				EvidenceID:           &validEvidenceID,
				EvidenceHash:         &validHash,
				EvidenceCompletedAt:  contentSupportString("2026-08-20T06:59:00Z"),
				GateSourceHash:       &validHash,
				SourceContentHash:    &validHash,
				RulesHash:            &validHash,
				ReleaseContentHash:   &validHash,
				...
				TestCoverage: &ContentTestCoverage{
					SchemaVersion: 1, Scope: "mini-mvp-l1", Required: 156, Passed: 156, Percent: 100,
				},
				MechanicsLocked: func() *bool { value := true; return &value }(),
			},
		},
		{
			name: "mini MVP evidence rejects a micro coverage scope",
			request: ContentSupportRequest{
				Status:               "verified_mechanical",
				CertificationVersion: contentSupportString(miniMVPEvidenceCertificationVersion),
				ContentHash:          &validHash,
