...
func conditionsAsEffects(db *sql.DB) error {
	queries := []string{
		...
		`INSERT INTO effects (name, description, card_number, effect_type, rarity, mechanics) VALUES
			...
			('Недееспособен','Нет действий/бонусных действий/реакций; концентрация прервана; помеха на инициативу.','COND-incapacitated','condition','common','{"effects":[{"resolution":"auto","result":[{"kind":"modifier","applies_to":{"roll":"initiative"},"op":"disadvantage"}]}]}'::jsonb),
			...
			('Парализован','Недееспособен; провал спасбросков СИЛ/ЛВК; атаки по вам вблизи — крит.','COND-paralyzed','condition','common','{"effects":[{"resolution":"auto","result":[{"kind":"modifier","applies_to":{"roll":"attack"},"op":"advantage","scope":"target"},{"kind":"modifier","applies_to":{"roll":"speed"},"op":"set","value":"0"}]}]}'::jsonb),
			...
			('Без сознания','Недееспособен, распластан; провал СИЛ/ЛВК; атаки по вам вблизи — крит.','COND-unconscious','condition','common','{"effects":[{"resolution":"auto","result":[{"kind":"modifier","applies_to":{"roll":"attack"},"op":"advantage","scope":"target"},{"kind":"modifier","applies_to":{"roll":"speed"},"op":"set","value":"0"}]}]}'::jsonb)
