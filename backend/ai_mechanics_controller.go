...
activation: {"mode":"active"|"passive"|"triggered", "cost":[{"resource":"action"|"bonus_action"|"reaction"}, {"resource":"spell_slot","level":N,"amount":1}]?, "trigger":{"event":"long_rest","timing":"after"}|{"events":["attack_roll_made","ability_check_made"],"timing":"after"}?}
...
- Межсуществовая реакция на к20: {"kind":"d20_interrupt","operation":"impose_disadvantage"|"subtract_die","timing":"before_roll"|"after_outcome","eligible_rolls":["attack_roll"|"ability_check"],"eligible_outcomes":["hit"|"success"]?,"range_ft":30,"requires_line_of_sight":true,"allowed_relations":["enemy"],"die":"1d6"|{"class":"bard","by_level":{"1":6,"5":8,"10":10,"15":12}}?}. Стоимости реакции и классового ресурса объявляются только в activation.cost.
...
- trigger.event или каждый элемент trigger.events СТРОГО: attack_roll_made|hit|sneak_attack_hit|miss|crit|damage_dealt|damage_taken|saving_throw_made|ability_check_made|reduced_to_0_hp|turn_start|turn_end|spell_cast|short_rest|long_rest.
...
- Эффекты на ДРУГИХ существ (бафф союзника, помеха врагу вне спасброска) → narrative, кроме строго подходящей реакции d20_interrupt.
