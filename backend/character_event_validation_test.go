...
func TestValidateCharacterEventAcceptsCurrentEngineEventUnion(t *testing.T) {
	valid := []JSONMap{
		...
		{"type": "stabilized", "source": "Spare the Dying"},
		{"type": "world_interaction", "operation": "beckon_water", "parameters": map[string]any{"created_water_cups": float64(1)}, "source": "Elementalism"},
		{"type": "communication", "mode": "reply", "sourceActorId": "target", "targetActorId": "caster", "private": true},
		{"type": "turn_started"},
		{"type": "turn_ended"},
		{"type": "short_rest"},
		{"type": "long_rest"},
...
func TestValidateCharacterEventRejectsMalformedOrAmbiguousPayloads(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "world interaction parameters are required object", eventType: "world_interaction", payload: JSONMap{"type": "world_interaction", "operation": "beckon_water", "parameters": []any{}}, want: "must be a JSON object"},
		{name: "communication must remain private", eventType: "communication", payload: JSONMap{"type": "communication", "mode": "message", "private": false}, want: "must be true"},
		{name: "communication mode is closed", eventType: "communication", payload: JSONMap{"type": "communication", "mode": "broadcast", "private": true}, want: "is unsupported"},
