...
const OUTPUT = join(FRONTEND_ROOT, 'src/canon/data/mini-mvp-fighting-style-fixture.v1.json');
...
async function main() {
  ...
  try {
    const module = await server.ssrLoadModule('/src/canon/miniMvpFightingStyleFixtureGenerator.ts');
    const artifact = await module.buildMiniMvpFightingStyleFixture(MINI_MVP_MANIFEST, {
      classes,
      races,
      backgrounds,
      feats,
      spells: spells.filter((spell) => miniSpellNumbers.has(spell.card_number)),
    });
    ...
    if (process.argv.includes('--check')) {
      ...
      console.log(`OK ${OUTPUT}: ${artifact.roots.length} Fighting Style roots`);
      return;
    }
    const temporary = `${OUTPUT}.${process.pid}.tmp`;
    await writeFile(temporary, rendered, 'utf8');
    await rename(temporary, OUTPUT);
    console.log(`WROTE ${OUTPUT}: ${artifact.roots.length} Fighting Style roots`);
  } finally {
    await server.close();
  }
}
