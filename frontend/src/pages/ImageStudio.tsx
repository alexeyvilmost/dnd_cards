  ...
  return (
    <div className="min-h-screen p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        <div className="site-page-head mb-6">
          <h1 className="site-heading text-2xl md:text-3xl font-bold text-gray-900 flex items-center gap-2">
            <Sparkles className="text-purple-600" /> Генерация изображений
          </h1>
          <p className="site-muted text-gray-600 mt-1 text-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5">
            <div className="site-surface bg-white rounded-xl shadow p-5 space-y-4">
              <div>
                <label className="site-muted block text-sm font-medium text-gray-700 mb-1">Концепт / название заклинания *</label>
                <input
                  value={subject}
                  onChange={(e) => setSubject(e.target.value)}
                  placeholder="напр. огненный шар, ледяная стрела, цепная молния"
                  className="site-control w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
              </div>

              <div>
                <label className="site-muted block text-sm font-medium text-gray-700 mb-1">Тип энергии (цвет)</label>
                <div className="flex items-center gap-2">
                  <span className="inline-block w-5 h-5 rounded-full border border-gray-300" style={{ backgroundColor: elementColor }} />
                  <select
                    value={element}
                    onChange={(e) => setElement(e.target.value)}
                    className="site-control flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                  >
                    {COLOR_TOKENS.map((c) => (
              ...
              <div>
                <label className="site-muted block text-sm font-medium text-gray-700 mb-1">Дополнительные детали (необязательно)</label>
                <input
                  value={extra}
              ...
              <div>
                <label className="site-muted block text-sm font-medium text-gray-700 mb-1">Качество</label>
                <select
                  value={quality}
              ...
              <div>
                <button
                  ...
                  className="site-muted text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1"
                >
                ...
                {showAdvanced && (
                  <textarea
                    ...
                    className="site-control mt-2 w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm"
                  />
                )}
              </div>
              ...
              <p className="site-muted text-xs text-gray-400">Генерация занимает 10–40 секунд и расходует кредиты OpenAI.</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-7">
            {results.length === 0 && !loading && (
              <div className="site-surface site-empty site-muted bg-white rounded-xl shadow p-10 text-center text-gray-400">
                Здесь появятся сгенерированные иконки
              </div>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {loading && (
                <div className="site-surface site-muted bg-white rounded-xl shadow flex items-center justify-center aspect-square text-gray-400">
                  <Loader2 className="animate-spin" size={32} />
                </div>
              )}
              {results.map((r, i) => (
                <div key={i} className="site-surface bg-white rounded-xl shadow overflow-hidden">
                  ...
                  <div className="p-3">
                    <div className="flex items-center justify-between">
                      <div className="site-label text-sm font-medium text-gray-800 truncate">{r.subject}</div>
                      <button
                        onClick={() => download(r.url, r.subject)}
                        aria-label="Скачать"
                        className="site-muted p-1.5 rounded hover:bg-gray-100 text-gray-600"
                      >
                        <Download size={16} />
                      </button>
                    </div>
                    <div className="site-muted text-xs text-gray-400 mt-0.5">{r.element} · {(r.ms / 1000).toFixed(1)}с</div>
                    <button
                    ...
                    {openPrompt === i && (
                      <p className="site-muted text-[11px] text-gray-500 mt-1 whitespace-pre-wrap break-words">{r.prompt}</p>
                    )}
                  </div>
                </div>
              ))}
