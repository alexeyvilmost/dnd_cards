...
describe('physical held-item removal',()=>{
 it('removes one accessible copy and preserves container contents and the input',()=>{
  const before=runtime(),saved=JSON.stringify(before);const next=dropHeldItem(before,'main_hand','owner',character);
  expect(JSON.stringify(before)).toBe(saved);expect(next.state.inventory).toEqual([{cardId:CARD_LONGSWORD.id,qty:1},{cardId:CARD_LONGSWORD.id,qty:3,containerId:'bag'}]);
 ...
 it('rejects a stale held reference backed only by an item inside a container',()=>{
  const state=runtime();state.inventory=state.inventory.filter(row=>row.containerId);expect(heldItemDropIssue(state,'main_hand')).toBeTruthy();
  expect(()=>dropHeldItem(state,'main_hand','owner',character)).toThrow();
 });
});
