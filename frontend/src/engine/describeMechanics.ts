...
const diceRu = (s: string, ctx?: FormulaContext | null) => formatFormulaDisplay(s, ctx);
...
function damagePhrase(p: Dict, ctx?: FormulaContext | null): string {
...
function payloadPhrase(p: Dict, ctx?: FormulaContext | null): string {
  switch (String(p.kind)) {
    case 'damage': return damagePhrase(p, ctx);
    ...
    case 'boon': return `талон ${diceRu(String(p.die ?? ''), ctx)}`;
...
function payloadsPhrases(arr: unknown, ctx?: FormulaContext | null): string[] {
  if (!Array.isArray(arr)) return [];
  return (arr as Dict[]).map((p) => payloadPhrase(p, ctx)).filter(Boolean);
}

function interactionPhrase(eff: Dict, ctx?: FormulaContext | null): string {
  const resolution = String(eff.resolution ?? '');
  if (resolution === 'attack_roll') {
    const on = [...payloadsPhrases(eff.on_hit, ctx), ...payloadsPhrases(eff.on_crit, ctx)];
    return `атака (к20)${on.length ? ` → ${on.join(', ')}` : ''}`;
  }
  if (resolution === 'save') {
    const abil = abilityRu(eff.ability);
    const dc = eff.dc != null ? ` СЛ ${diceRu(String(eff.dc), ctx)}` : '';
    const on = payloadsPhrases(eff.on_fail, ctx);
  ...
  // auto
  return payloadsPhrases(eff.result ?? eff.results, ctx).join(', ');
}
...
export function describeMechanics(
  ...
  formulaCtx?: FormulaContext | null,
  ...
  const summaryParts = Array.isArray(effects)
    ? effects.map((eff) => interactionPhrase(eff, formulaCtx)).filter(Boolean)
    : [];
...
export function describeMechanicsLine(
  mechanics: Dict | null | undefined,
  formulaCtx?: FormulaContext | null,
): string {
  const d = describeMechanics(mechanics, formulaCtx);
