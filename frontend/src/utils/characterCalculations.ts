// Утилиты для расчетов характеристик персонажа V2
...
export const getPassivePerception = (wisdom: number, isProficient: boolean, level: number): number => {
...
export const hasSkillProficiency = (character: CharacterV2, skill: string): boolean => {
  return character.skill_proficiencies.includes(skill);
};

// Проверка владения спасброском
export const hasSavingThrowProficiency = (character: CharacterV2, stat: string): boolean => {
  return character.saving_throw_proficiencies.includes(stat);
};

// Получение значения характеристики
export const getStatValue = (character: CharacterV2, stat: string): number => {
...
export const calculateDerivedStats = (character: CharacterV2) => {
  ...
  return {
    ...
    // Спасброски
    savingThrows: {
      strength: getSavingThrow(character.strength, hasSavingThrowProficiency(character, 'strength'), character.level),
      dexterity: getSavingThrow(character.dexterity, hasSavingThrowProficiency(character, 'dexterity'), character.level),
      constitution: getSavingThrow(character.constitution, hasSavingThrowProficiency(character, 'constitution'), character.level),
      intelligence: getSavingThrow(character.intelligence, hasSavingThrowProficiency(character, 'intelligence'), character.level),
      wisdom: getSavingThrow(character.wisdom, hasSavingThrowProficiency(character, 'wisdom'), character.level),
      charisma: getSavingThrow(character.charisma, hasSavingThrowProficiency(character, 'charisma'), character.level),
    },
