...
describe('paper sheet library references', () => {
  ...
  it('appends an entity from a note header and replaces only the editor selection while preserving text and resource counters', async () => {
    ...
    const editor = labelled<HTMLTextAreaElement>('Текст: Снаряжение, строка 1');
    ...
    const returnedEditor = labelled<HTMLTextAreaElement>('Текст: Снаряжение, строка 1');
    expect(returnedEditor.value).toBe(replaced.split('\n')[0]);
  ...
  it('cancels library selection without changing note text and returns to the editor', async () => {
    ...
    const returnedEditor = labelled<HTMLTextAreaElement>('Текст: Умения, строка 1');
  ...
  it('uses the active inline caret when the note header opens the library', async () => {
    ...
    const editor = labelled<HTMLTextAreaElement>('Текст: Умения, строка 2');
    ...
    const returnedEditor = labelled<HTMLTextAreaElement>('Текст: Умения, строка 2');
    expect(returnedEditor.value).toBe(expected.split('\n')[1]);
    const caret = 3 + paperEntityToken(ENTITIES[0]).length;
    expect([returnedEditor.selectionStart, returnedEditor.selectionEnd]).toEqual([caret, caret]);
