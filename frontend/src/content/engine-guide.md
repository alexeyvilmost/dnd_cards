...
| Dummy-цель | Только КД и модификаторы спасбросков без изменяемого состояния. |
...
flowchart TD
    ...
    Resolver --> Derived["HP, КД, скорость, инициатива, груз"]
...
| КД | Базовый метод, методы без доспеха, `set_value ac_base`, модификаторы; экипировка добавляется позднее в breakdown |
...
- максимум HP;
- КД без полного предметного breakdown;
...
| `sneak_attack_hit` | После попадания, на котором впервые за ход сработала точная библиотечная `EFF-sneak-attack` (боевой адаптер) |
...
| `weapon_profile` | object | Единственный runtime-authority оружия, включая `ammo: null | {card_id,name?}` |
...
| Установить альтернативную КД | `set_value` с `target: "ac_base"` |
...
Путь: предмет/эффект → пассивы → breakdown КД → отображение на обоих листах.
...
Выбранная Card длинного лука отдельно обязана объявить полный
`mechanics.weapon_profile`, включая ranged mode 150/600, свойство `ammunition`
и `ammo: {"card_id":"arrow","name":"Стрела"}`. Actor binder заменяет
контекстный маркер обычной item-ценой до показа target UI. Путь: движок
атомарно списывает действие и стрелу → бросок против КД → урон цели → журнал и
сохранение. `Card.range`, tags и имя не участвуют в исполнении.
