...
export function createSheetCombatRuntime(dependencies: SheetCombatDataSource) {
 


function resolveSheetCombatArmorClass(
 ...
 */
async function collectSheetCombatActionInventory(input: {
  ...
  const resolveAction = input.resolveAction ?? dependencies.actionsApi.getAction;
  const resolveEffect = input.resolveEffect ?? dependencies.effectsApi.getEffect;
  ...
  const masteryRows = input.masteryCatalog
    ?? (input.requiresMasteryCatalog
      ? await (input.loadMasteryCatalog ?? dependencies.loadMasteryEffectsStrict)()
      : []);
 ...
 */
async function loadSheetCombatParticipant(input: {
