...
import type { createRegistry } from '../engine/registry';
...
export interface AssemblyDataSource {
  racesApi: Pick<typeof import('../api/client')['racesApi'], 'getRace'>;
  classesApi: Pick<typeof import('../api/client')['classesApi'], 'getClass'>;
  backgroundsApi: Pick<typeof import('../api/client')['backgroundsApi'], 'getBackground'>;
  featsApi: Pick<typeof import('../api/client')['featsApi'], 'getFeat'>;
  effectsApi: Pick<typeof import('../api/client')['effectsApi'], 'getEffect' | 'getEffects'>;
...
export function createAssemblyRuntime(dependencies: AssemblyDataSource) {
 ...
 * so adding another `choice(dependencies: "effect" | "feat")` needs no UI branch.
 */
function bundleDependencyKey(
...
function assemble(bundle: EntityBundle, draft: CharacterDraft): AssembledCharacter {
  ...
  for (const klass of (bundle.classes ?? []).filter((entry) => entry.id !== draft.classId)) {
    for (const choice of klass.multiclass_proficiencies?.choices ?? []) {
      pendingChoices.push({
        id: choice.id,
        ...
        grant: choice.grant,
        origin: { kind: 'class', id: klass.id, name: `${klass.name} (мультикласс)` },
      });
    }
  }
  ...
  return {
    ...
    derived: {
      proficiencyBonus: pb,
      maxHP: computeMulticlassMaxHP(
        (bundle.classes ?? (bundle.klass ? [bundle.klass] : [])).map((klass) => ({
          id: klass.id,
          hit_die: klass.hit_die,
          level: draftClassLevels(draft)[klass.id] ?? (klass.id === (draft.classId ?? bundle.klass?.id) ? draft.level : 0),
        })),
        draft.classId ?? bundle.klass?.id,
        scores.con,
      ),
...
async function loadBundle(draft: CharacterDraft): Promise<EntityBundle> {
  // B5: справочник переменных ни от чего не зависит — стартуем сразу, ждём в конце.
  const variableDefsP = dependencies.variablesApi
    .getVariables()
    .then((r) => r.variables ?? [])
    .catch(() => []);
  ...
  const [race, klass, secondaryClassesRaw, background, manualFeatsRaw, subrace, subclassesRaw] = await Promise.all([
    draft.raceId ? dependencies.racesApi.getRace(draft.raceId) : Promise.resolve(null),
    draft.classId ? dependencies.classesApi.getClass(draft.classId) : Promise.resolve(null),
    Promise.all(secondaryClassIds.map((id) => dependencies.classesApi.getClass(id).catch(() => null))),
    draft.backgroundId ? dependencies.backgroundsApi.getBackground(draft.backgroundId) : Promise.resolve(null),
    Promise.all((draft.featIds || []).map((id) => dependencies.featsApi.getFeat(id))),
    draft.lineageId && isEntityUuid(draft.lineageId) ? dependencies.racesApi.getRace(draft.lineageId) : Promise.resolve(null),
    Promise.all(selectedSubclassIds.map((id) => (
      isEntityUuid(id) ? dependencies.classesApi.getClass(id).catch(() => null) : Promise.resolve(null)
    ))),
  ]);
  ...
  const originFeat = !draft.swapFeat && background?.origin_feat
    ? await dependencies.featsApi.getFeat(background.origin_feat)
    : null;
  ...
  const combinedClassResources: Record<string, unknown> = {};
  ...
  await Promise.all(
    uniqueEffectIds.map((id) => dependencies.effectsApi.getEffect(id).then((e) => { effBodyById.set(id, e); }).catch(() => {})),
  );
  ...
  // Материализуем choice(dependencies:"effect_type"): выбор N эффектов заданного
  // типа (Дар договора, Мистическое воззвание). Тип разворачивается в обычный
  // choice(dependencies:"effect") со списком эффектов этого типа — дальше работает
  ...
  // Черты, выбранные через choice(dependencies:"feat") в механике эффектов (боевой
  ...
  const chosenById = new Map<string, Feat>(
    (await Promise.all([...new Set(chosenFeatPicks.map((p) => p.featId))].map((id) => dependencies.featsApi.getFeat(id).catch(() => null))))
  ...
  const getEff = async (id: string) => {
    if (!effCache.has(id)) effCache.set(id, await dependencies.effectsApi.getEffect(id).catch(() => null));
  ...
  const actions = (
    await Promise.all(
      actionRefs.map((r) =>
        dependencies.actionsApi.getAction(r.id).then((action) => ({
          action,
  ...
  const [variableDefs, resources] = await Promise.all([
    variableDefsP,
    Promise.all((draft.resourceIds || []).map((id) => dependencies.resourcesApi.getResource(id))),
  ]);
...
const entityRegistry = dependencies.entityRegistry;
...
function collectEffectGrantRefs(
...
function collectFeatChoiceRefs(
  mechanics: RefDict | null | undefined,
  effectId: string,
  origin: ChoiceOrigin,
  draft: CharacterDraft,
...
async function expandEffectGrants(
  base: OriginEffect[],
  draft: CharacterDraft,
 ...
 * выданный эффект наследует item-семантику (подавление числовых ролей, фильтр КД) из слайса 1.
 */
async function expandItemGrantedEffects(
...
async function loadAssembly(draft: CharacterDraft): Promise<AssembledCharacter> {
  ...
  const uuidSpells = await Promise.all(uuids.map((id) => dependencies.spellsApi.getSpell(id).catch(() => null)));
  ...
  return {multiclassSpellSlotCounts, gatherFeatureRefs, bundleDependencyKey, assemble, loadBundle, collectEffectGrantRefs, collectFeatChoiceRefs, expandEffectGrants, expandItemGrantedEffects, loadAssembly};
}
