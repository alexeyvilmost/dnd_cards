...
describe('resolveCharacterRules — вложенные выборы (фундамент ASI)', () => {
  ...
  it('повторяемая черта на двух слотах: два НЕЗАВИСИМЫХ прироста (ключи-на-экземпляр, 3b)', () => {
    // Один ASI-эффект, но два получения (разные instanceKey слотов-пикеров): вложенные
    // выборы не сталкиваются — +2 СИЛ на одном слоте, +2 ЛОВ на другом.
    const inst = (key: string): ChoiceOrigin => ({ kind: 'feat', id: 'asi', name: 'ASI', instanceKey: key });
    const rs = build({
      effects: [
        fx('asi_fx', { effects: [asiChoice] }, inst('slot1')),
        fx('asi_fx', { effects: [asiChoice] }, inst('slot2')),
      ],
      draft: { resolvedChoices: {
        'feat:asi:asi_fx#slot1:asi_mode': ['plus2'],
        'feat:asi:asi_fx#slot1:asi_p2': ['str'],
        'feat:asi:asi_fx#slot2:asi_mode': ['plus2'],
        'feat:asi:asi_fx#slot2:asi_p2': ['dex'],
      } },
    });
    expect(rs.abilities.str).toBe(STD.str + 2);
    expect(rs.abilities.dex).toBe(STD.dex + 2);
  });
