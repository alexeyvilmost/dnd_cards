...
/**
 * Browser-safe synchronous SHA-256 for identities that must be constructed
 * inside render-time deterministic adapters. Web Crypto remains the preferred
 * asynchronous boundary; this implementation exists so a wire identity is
 * never downgraded to a short non-cryptographic fingerprint.
 */
...
export function canonicalSha256Sync(value: unknown): string {
  return `sha256:${sha256String(canonicalStringify(value))}`;
}
