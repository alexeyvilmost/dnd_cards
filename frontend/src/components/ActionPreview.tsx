...
const ActionPreview = ({ action, className = '', disableHover = false, onClick }: ActionPreviewProps) => {
  const isExtended = Boolean(action.is_extended);
  ...
  return (
    <div 
      className={`card-preview relative bg-white rounded-lg shadow-md overflow-hidden ${getBorderColor(action.rarity)} border-4 ${className} transition-all duration-300 ease-out group ${getRarityGlowColor(action.rarity)} ${isExtended ? 'w-[397px] h-[280px]' : 'w-[198px] h-[280px]'} ${!disableHover ? 'hover:scale-105 hover:-translate-y-2 hover:shadow-2xl' : ''} flex flex-col ${onClick ? 'cursor-pointer' : ''}`}
      onClick={onClick}
    >
      {/* Метка редкости */}
      <div className="absolute top-0.5 left-1 text-sm font-bold select-none">
        <span 
          title={getRaritySymbolDescription(action.rarity)}
          aria-label={getRaritySymbolDescription(action.rarity)}
          className={`${getRarityColor(action.rarity)} drop-shadow-lg`}
          style={{ textShadow: '1px 1px 2px rgba(0,0,0,0.8)' }}
        >
          {getRaritySymbol(action.rarity)}
        </span>
      </div>
      ...
      {isExtended ? (
        ...
        <>
          <div className="flex flex-1">
            ...
            <div className="w-1/2 flex flex-col">
              <div className="px-1 py-0.5 text-center">
                <h3 className={getTitleClass(action.rarity, action.name)}>
                  {action.name}
                </h3>
              </div>
              ...
              <div className="px-2 pt-0 pb-2 bg-gray-50 flex-1 min-h-[60px]">
                <div className="text-xs text-gray-600 space-y-1">
                  <div><strong>Ресурс:</strong> {getResourceLabel(action.resource)}</div>
                  <div><strong>Тип:</strong> {getActionTypeLabel(action.action_type)}</div>
                  {action.recharge && (
                    <div><strong>Перезарядка:</strong> {getRechargeLabel(action.recharge)}{action.recharge === 'custom' && action.recharge_custom ? ` (${action.recharge_custom})` : ''}</div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="w-1/2 px-2 pt-2 pb-2 bg-white flex-1 flex flex-col">
              <div className="flex-1 overflow-y-auto">
                <p 
                  ...
                  style={{
                    fontSize: action.description_font_size ? `${action.description_font_size}px` : '12px',
                    textAlign: (action.text_alignment || 'center') as 'left' | 'center' | 'right'
                  }}
                >
          ...
          {/* Нижняя часть - номер карты и дополнительная информация */}
          <div className="absolute bottom-2 left-2 right-2 flex justify-between items-center text-xs">
            <span className={`font-mono ${action.card_number ? 'text-gray-900' : 'text-gray-400'}`}>
              {action.card_number || 'Без номера'}
            </span>
          </div>
        </>
      ) : (
        ...
        <>
          ...
          <div className="px-2 pt-1 pb-2 bg-gray-50 flex-1 min-h-[60px] overflow-y-auto">
            <p 
              className={`text-xs font-fantasy whitespace-pre-wrap`}
              style={{
                fontSize: action.description_font_size ? `${action.description_font_size}px` : '10px',
                textAlign: (action.text_alignment || 'center') as 'left' | 'center' | 'right'
              }}
            >
              {action.description || 'Нет описания'}
          ...
          <div className="absolute bottom-2 left-2 right-2 flex justify-between items-center text-xs">
            ...
            <div className="text-gray-600 text-[10px]">
              {getResourceLabel(action.resource)}
            </div>
          </div>
        </>
      )}
