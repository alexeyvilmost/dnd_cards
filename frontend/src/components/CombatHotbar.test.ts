...
function state(freeUses: number, actions = 1): SoloCombatState {
  return {
    characterId: 'hero',
    world: {
      actors: {
        hero: {
          ...
          runtime: { hp: { current: 10, max: 10, temp: 0 }, resources: { action: actions, 'freeuse-spell': freeUses }, maxResources: { action: 1, 'freeuse-spell': 1 }, activeEffects: [], equipment: {}, inventory: [], firedThisTurn: [] },
